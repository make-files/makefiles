...
vitest: $(JS_VITEST_REQ) $(_JS_VITEST_REQ)
	$(JS_EXEC) vitest $(_JS_VITEST_ARGS)$(if $(JS_VITEST_FORBID_ONLY), --allowOnly=false)$(shell for p in $(JS_VITEST_PROJECTS); do printf ' --project=%q' "$$p"; done)
...
artifacts/coverage/vitest/index.html: $(JS_VITEST_REQ) $(_JS_VITEST_REQ)
	$(JS_EXEC) vitest $(_JS_VITEST_ARGS)$(if $(JS_VITEST_FORBID_ONLY), --allowOnly=false) --coverage.enabled --coverage.reportsDirectory="$(@D)" --coverage.reporter=text --coverage.reporter=html$(shell for p in $(JS_VITEST_PROJECTS); do printf ' --project=%q' "$$p"; done)

.PHONY: artifacts/coverage/vitest/lcov.info # always rebuild
artifacts/coverage/vitest/lcov.info: $(JS_VITEST_REQ) $(_JS_VITEST_REQ)
	$(JS_EXEC) vitest $(_JS_VITEST_ARGS)$(if $(JS_VITEST_FORBID_ONLY), --allowOnly=false) --coverage.enabled --coverage.reportsDirectory="$(@D)" --coverage.reporter=text --coverage.reporter=lcovonly$(shell for p in $(JS_VITEST_PROJECTS); do printf ' --project=%q' "$$p"; done)
